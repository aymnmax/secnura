...
function SecurityShowcase() {
  ...
  // Keep the hero lively on desktop while remaining easy to read on mobile.
  useEffect(() => {
  ...
  function handlePointerUp(event: PointerEvent<HTMLDivElement>) {
    ...
    if (Math.abs(delta) > 45) {
      changeService(activeIndex + (delta < 0 ? 1 : -1));
    }
    setDragStart(null);
  }
  ...
  return (
    ...
    >
      ...
      <div className="showcase-carousel" aria-live="polite">
        <div className="showcase-carousel-head">
          <span className="showcase-carousel-label">SERVICE {serviceNumber} / {String(services.length).padStart(2, "0")}</span>
          <span className="showcase-carousel-hint">AUTO-ROTATING · SWIPE</span>
        </div>

        <article className="showcase-feature-card" key={activeService.slug}>
          <div className="showcase-feature-index">{serviceNumber}</div>
          ...
          <div className="showcase-feature-copy">
            ...
            <a href="#contact">Discuss this service <b>→</b></a>
          </div>
          <div className="showcase-feature-badge">
            <span>SECURITY FOCUS</span>
            <strong>{activeService.shortName.toUpperCase()}</strong>
          </div>
        </article>
